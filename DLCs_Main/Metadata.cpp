...
void DLCMetadata::Load(const IniFile* ini)
{
	...
	for (int s = 0; s < 16; s++)
	{
		...
		FrenchStrings[s] = UTF8ToANSI(ini->getString("", "FrenchMessage" + std::to_string(s), ""), FrenchCodepage);
		SpanishStrings[s] = UTF8ToANSI(ini->getString("", "SpanishMessage" + std::to_string(s), ""), SpanishCodepage);
		GermanStrings[s] = UTF8ToANSI(ini->getString("", "GermanMessage" + std::to_string(s), ""), GermanCodepage);
	}
}
